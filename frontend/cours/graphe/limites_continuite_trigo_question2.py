...
# Tracer (1 - cos(x))/x^2
plt.plot(x1, f(x1), 'b-', linewidth=1.5, label=r'$f(x) = \frac{1 - \cos(x)}{x^2}$')
plt.plot(x2, f(x2), 'b-', linewidth=1.5)

# Point limite en (0, 1/2) - prolongement par continuité
plt.plot(0, 0.5, 'go', markersize=8, markeredgecolor='darkgreen', markeredgewidth=2, label=r'$\lim_{x \to 0} \frac{1 - \cos(x)}{x^2} = \frac{1}{2}$', zorder=5)

# Ligne horizontale y = 1/2 pour visualiser la limite
plt.axhline(y=0.5, color='green', linestyle='--', linewidth=1, alpha=0.5)
...
# Graduation spéciale pour y = 0.5
ax.plot([-0.3, 0.3], [0.5, 0.5], color="green", linewidth=0.8)
ax.text(-1.5, 1.0, r'$\frac{1}{2}$', ha='right', va='center', fontsize=10, color='green')
...
# Sauvegarder
plt.savefig('limites_continuite_trigo_question2.png', dpi=300, bbox_inches='tight')
print("Graphique 'limites_continuite_trigo_question2.png' créé avec succès!")
