...
# Domaine de définition (e^x - 1 > 0, donc x > 0)
x = np.linspace(0.1, 20, 1500)

# Courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = \ln(e^x - 1)$')

# Ligne y = 1
plt.axhline(y=1, color='red', linestyle='--', linewidth=2, label='$y = 1$')

# Solution
x_sol = np.log(np.exp(1) + 1)
plt.plot(x_sol, 1, 'go', markersize=8, label=f'Solution $x = \\ln(e + 1)$')

# Zone où f(x) >= 1
x_zone = x[x >= x_sol]
ax.fill_between(x_zone, f(x_zone), 1, where=(f(x_zone) >= 1), 
...
# Sauvegarde
plt.savefig('exercice_exponentielle_logarithmiques_question5.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_logarithmiques_question5.png' créé avec succès!")
