...
# Courbe y = ln(x)
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$y = \ln(x)$')

# Droite y = 2
plt.axhline(y=2, color='orange', linestyle='--', linewidth=1.5, label=r'$y=2$')

# Asymptote verticale x=0
plt.axvline(x=0, color='red', linestyle='--', linewidth=1.5, label=r'Asymptote $x=0$')

# Point solution x = e^2
x_sol = np.e**2
y_sol = 2
plt.plot(x_sol, y_sol, 'mo', markersize=6, label=r'Solution $x=e^2$')
...
plt.savefig('exercice_logarithme_equations_question2.png', dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_logarithme_equations_question2.png' créé avec succès!")
