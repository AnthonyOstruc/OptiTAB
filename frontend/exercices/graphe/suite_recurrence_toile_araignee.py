...
plt.plot(x, y_f, label="f(x) = 1/2 x + 1")
plt.plot(x, y_id, linestyle='--', label="Droite y = x")

# Toile d'araignée pour u0 -> u4
for n in range(4):
    # segment vertical : (u_n, u_n) -> (u_n, f(u_n)) = (u_n, u_{n+1})
    x_vert = [u[n], u[n]]
    y_vert = [u[n], u[n+1]]
...
# Points (u_n, u_n) avec étiquettes u0, u1, ...
for n, un in enumerate(u[:-1]):  # u0 à u4
    plt.scatter(un, un, zorder=5)
    plt.text(un + 0.05, un - 0.11, f"u{n}", fontsize=8)
