...
# Fonction g(x) = (2x + 1)/(x + 3)
def g(x):
    return (2*x + 1) / (x + 3)

# Domaine de définition (éviter x = -3)
x1 = np.linspace(-20, -3.05, 1500)
x2 = np.linspace(-2.95, 20, 1500)

# Courbes
plt.plot(x1, g(x1), 'b-', linewidth=2, label=r'$g(x) = \frac{2x + 1}{x + 3}$')
plt.plot(x2, g(x2), 'b-', linewidth=2)

# Asymptote verticale x = -3
plt.axvline(x=-3, color='red', linestyle='--', linewidth=1.5, alpha=0.7, label='Asymptote verticale $x = -3$')

# Asymptote horizontale y = 2
plt.axhline(y=2, color='green', linestyle='--', linewidth=1.5, alpha=0.7, label='Asymptote horizontale $y = 2$')
...
# Sauvegarde
plt.savefig('/Users/anthonytabet/Desktop/OptiTABV2/OptiTAB/frontend/cours/graphe/asymptotes_question2.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'asymptotes_question2.png' créé avec succès!")
