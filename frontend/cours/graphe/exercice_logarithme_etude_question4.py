...
# Echantillonnage très dense près de x=0
near_zero = np.geomspace(1e-6, 0.5, 900)
far_segment = np.linspace(0.5, 20.0, 700)
x = np.concatenate([near_zero, far_segment])

# Courbe principale
plt.plot(x, k(x), 'b-', linewidth=2, label=r'$k(x) = \frac{\ln(x)}{x^2}$')

# Point maximum
x_max = np.sqrt(np.e)
y_max = k(x_max)
plt.plot(x_max, y_max, 'ro', markersize=8, label=f'Maximum en $x = \\sqrt{{e}}$ : $k(\\sqrt{{e}}) = \\frac{{1}}{{2e}}$')

# Asymptote horizontale
plt.axhline(y=0, color='green', linestyle='--', linewidth=2, label='Asymptote horizontale $y = 0$')
...
# Sauvegarde
plt.savefig('exercice_logarithme_etude_question4.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_logarithme_etude_question4.png' créé avec succès!")
