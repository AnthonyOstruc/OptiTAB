...
# Fonction g(x) = sin(2x)/x
def g(x):
    return np.sin(2*x) / x

# Domaine de définition (éviter x = 0)
x1 = np.linspace(-20, -0.05, 1500)
x2 = np.linspace(0.05, 20, 1500)

# Courbes
plt.plot(x1, g(x1), 'b-', linewidth=2, label=r'$g(x) = \frac{\sin(2x)}{x}$')
plt.plot(x2, g(x2), 'b-', linewidth=2)

# Trou en x = 0, y = 2 (discontinuité évitable)
plt.plot(0, 2, 'ro', markersize=8, markerfacecolor='white', markeredgewidth=2, label='Trou en $(0, 2)$')

# Point de prolongement
plt.plot(0, 2, 'go', markersize=6, label='Prolongement : $g(0) = 2$')

# Annotation
ax.annotate('$g(0) = 2$', (0, 2), xytext=(-5, 5), fontsize=10,
...
# Sauvegarde
plt.savefig('/Users/anthonytabet/Desktop/OptiTABV2/OptiTAB/frontend/cours/graphe/prolongement_question2.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'prolongement_question2.png' créé avec succès!")
