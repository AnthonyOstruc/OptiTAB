...
# Courbes
plt.plot(x1, f1(x1), 'b-', linewidth=2, label=r'$y = x^3$')
plt.plot(x1, f2(x1), 'r-', linewidth=2, label=r'$y = x$')

# Remplissage de l'aire entre les courbes - Partie 1 (-1 à 0)
x_fill1 = np.linspace(a, intersection, 50)
y1_fill1 = f1(x_fill1)
y2_fill1 = f2(x_fill1)

# Création du polygone pour la première partie
vertices1 = list(zip(x_fill1, y1_fill1)) + list(zip(x_fill1[::-1], y2_fill1[::-1]))
polygon1 = Polygon(vertices1, facecolor='lightgreen', alpha=0.7, edgecolor='green', linewidth=2)
ax.add_patch(polygon1)

# Remplissage de l'aire entre les courbes - Partie 2 (0 à 1)
x_fill2 = np.linspace(intersection, b, 50)
y1_fill2 = f1(x_fill2)
y2_fill2 = f2(x_fill2)

# Création du polygone pour la deuxième partie
vertices2 = list(zip(x_fill2, y2_fill2)) + list(zip(x_fill2[::-1], y1_fill2[::-1]))
...
plt.axvline(x=intersection, color='orange', linestyle=':', linewidth=1.5, alpha=0.7, label='$x = 0$')
...
aire = 1/2
plt.text(0.02, 0.98, f'$A = \\frac{{1}}{{2}} = {aire}$', 
         transform=plt.gca().transAxes, fontsize=14, fontweight='bold',
         bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.8),
         verticalalignment='top')

# Sauvegarde
plt.savefig('/Users/anthonytabet/Desktop/OptiTABV2/OptiTAB/frontend/cours/graphe/aire_entre_courbes_question5.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'aire_entre_courbes_question5.png' créé avec succès!")
