...
# Courbe
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$y = e^x$')

# Ligne horizontale y = 7
plt.axhline(y=7, color='red', linestyle='--', linewidth=2, label='$y = 7$')

# Zone où f(x) >= 7 (solution de l'inéquation)
x_zone = np.linspace(np.log(7), 20, 1000)
plt.fill_between(x_zone, f(x_zone), 7, alpha=0.3, color='green', 
                 label='$e^x \\geq 7$')

# Point limite (solution de l'inéquation)
plt.plot(np.log(7), 7, 'go', markersize=6, label='Limite $x = \\ln(7)$')
...
# Sauvegarde
plt.savefig('exercice_exponentielle_equations_question5.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_equations_question5.png' créé avec succès!")