import{a as p,c as tt,A as et,b as d,a1 as at,d as n,g as b,k as D,B as U,L as it,F as k,p as T,t as g,H as ot,o as u,n as S,j as rt}from"./vendor-JoY6QM7x.js";import{_ as nt}from"./index-CaRFDJpf.js";import{a as lt,g as st}from"./curriculum-1vPkSFMM.js";import{r as dt,g as ut,u as gt,c as ct,e as pt,a as mt}from"./scientificRenderer-Bx8oYBxt.js";import"./ui-jaH5oyHW.js";import"./pdf-B39ievqC.js";import"./mathlive-B5k0jhbN.js";const vt={class:"bulk-form"},ft=["value"],bt={class:"images-upload-section"},ht={key:0,class:"selected-images"},xt=["src","alt"],qt={class:"image-name"},yt=["onClick"],It={class:"btn-group"},Ct=["disabled"],Et=["disabled"],_t={key:0,class:"success-msg"},At={key:1,class:"error-msg"},$t={key:2,class:"info-msg"},kt={key:3,class:"preview-section"},Tt={key:0,class:"preview-image-info"},Lt={class:"image-indicator"},Nt={class:"image-status-list"},Dt={class:"preview-cours"},Mt={class:"preview-header"},Ot={class:"ordre-badge"},Pt={key:0,class:"preview-description"},wt=["innerHTML"],Rt=10*1024*1024,Ut={__name:"AdminCoursPlus",setup(St){const j=["image/jpeg","image/jpg","image/png","image/gif","image/webp","image/svg+xml"],y=p([]),_=p([]),A=p(""),c=p(""),f=p(""),L=p(""),v=p(""),h=p([]),M=p(!1),x=p([]),N=p(null);class z{constructor(){this.images=new Map}addImage(e){if(!this.validateImage(e))throw new Error(`Image invalide: ${e.name}`);this.images.set(e.name,e)}removeImage(e){this.images.delete(e)}validateImage(e){return!(!j.includes(e.type)||e.size>Rt)}getImage(e){return this.images.get(e)}getImageNames(){return Array.from(this.images.keys())}}const I=new z;function O(a){return _.value.find(e=>String(e.id)===String(a))}function F(a){if(!a)return null;const e=O(a.notion);if(!e)return{themeNom:"",matiereNom:"",paysNom:"",niveauNom:""};const t=e.matiere_nom||e.contexte_detail&&e.contexte_detail.matiere_nom||"",r=e.theme_nom||"",o=e.contexte_detail&&e.contexte_detail.pays?e.contexte_detail.pays.nom:"",s=e.contexte_detail&&e.contexte_detail.niveau?e.contexte_detail.niveau.nom:"";return{matiereNom:t,themeNom:r,paysNom:o,niveauNom:s}}function P(a){const e=O(a.notion),t=F(a);return[a.nom,e?`— ${e.nom}`:"",t&&t.matiereNom?`— ${t.matiereNom}`:"",t&&(t.paysNom||t.niveauNom)?`— ${[t.paysNom,t.niveauNom].filter(Boolean).join(" · ")}`:""].filter(Boolean).join(" ")}function V(a){return{easy:"Facile",medium:"Moyen",hard:"Difficile"}[a]||a}function B(a){return URL.createObjectURL(a)}function $(a){return I.getImage(a)}function H(a,e=null){return(a||"").split(",").map(r=>r.trim()).filter(Boolean).map((r,o)=>{const s=$(r);return{id:`preview-${o}`,image:s?URL.createObjectURL(s):r,image_type:"illustration",position:o+1}})}function W(a){const e=H(a.image,a);let t=a.contenu;const r=(a.image||"").split(",").map(o=>o.trim()).filter(Boolean);return t=t.replace(/\[IMAGE_(\d+)\]/g,(o,s)=>{const i=parseInt(s)-1,l=r[i],m=$(l);return m?`
...
`),!t.titre||!t.contenu||!t.matiere||!t.chapitre?null:t}function X(){try{M.value=!0,h.value=w(f.value),ot(()=>{dt()})}catch{v.value="Erreur lors de la prévisualisation"}}async function Y(){if(!c.value){v.value="Veuillez sélectionner un chapitre";return}const a=y.value.find(t=>t.id==c.value);if(!a){v.value="Chapitre invalide";return}if(!_.value.find(t=>t.id==a.notion)){v.value="Notion invalide pour ce chapitre";return}try{const t=w(f.value);if(t.length===0){v.value="Aucun cours valide trouvé";return}let r=0,o=0,s=0,i=null;try{const l=await ut(null,null,Number(c.value)),m=Array.isArray(l?.data)?l.data:Array.isArray(l)?l:[];m&&m.length>0&&(i=m[0].id)}catch{}for(const l of t)try{const m={chapitre:Number(l.chapitre),titre:l.titre,contenu:l.contenu,ordre:l.ordre||0,difficulty:l.difficulty||"medium"};let C;if(i)C=(await gt(i,m))?.id||i,o++;else{const{data:E}=await ct(m);C=E?.id,r++,i=C}if(l.image&&C){const E=l.image.split(",").map(q=>q.trim()).filter(Boolean);for(let q=0;q<E.length;q++){const R=I.getImage(E[q]);if(R){const K={cours:C,image:R,image_type:"illustration",position:q+1};await pt(K)}}}}catch{s++}if(r>0||o>0){L.value=`${r} créé(s)${o?`, ${o} mis à jour`:""}${s>0?`, ${s} erreur(s)`:""}`;const l=c.value;f.value="",h.value=[],x.value=[],I.images.clear(),N.value&&(N.value.value=""),c.value=l}else v.value="Aucun cours n'a pu être créé"}catch{v.value="Erreur lors de la création des cours"}}const Z=tt(()=>{if(!A.value)return y.value;const a=A.value.toLowerCase();return y.value.filter(e=>P(e).toLowerCase().includes(a))});return et(async()=>{try{const[a,e]=await Promise.all([lt(),st()]);y.value=Array.isArray(a)?a:a?.data||[],_.value=Array.isArray(e)?e:e?.data||[]}catch{}}),(a,e)=>(u(),d("div",null,[e[8]||(e[8]=at(`<h2 class="admin-title" data-v-3a1bc272>Bulk – Ajout de Cours</h2><div class="format-help" data-v-3a1bc272><h3 data-v-3a1bc272>📋 Format requis :</h3><div class="format-example" data-v-3a1bc272><pre data-v-3a1bc272><code data-v-3a1bc272>=== [NOM DU COURS - SOUS-TITRE]
