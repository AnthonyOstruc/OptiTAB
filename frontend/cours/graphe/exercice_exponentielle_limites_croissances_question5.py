...
# Courbes principales
plt.plot(x, f(x), 'b-', linewidth=2, label=r"$f(x) = e^x - x$")

# g(x) défini sur (0, +inf)
x_pos = x[x > 0]
plt.plot(x_pos, g(x_pos), 'r-', linewidth=2, label=r"$g(x) = \dfrac{e^x}{x}$")

# Points caractéristiques
plt.plot(0, f(0), 'go', markersize=8, label=r"Minimum de $f$ en $x = 0$ : $f(0) = 1$")
plt.plot(1, g(1), 'mo', markersize=8, label=r"Minimum de $g$ en $x = 1$ : $g(1) = e$")
...
# Sauvegarde
plt.savefig('exercice_exponentielle_limites_croissances_question5.png', dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_limites_croissances_question5.png' créé avec succès!")
