...
# Fonctions
def h(x):
    return np.log(x**2 - 3*x + 2)

# Domaine de définition : ]-∞, 1[ ∪ ]2, +∞[
# Créer deux domaines séparés
x1 = np.linspace(-20, 0.99999999, 20000)  # Premier intervalle - ultra proche de 1
x2 = np.linspace(2.00000000001, 20, 20000)   # Deuxième intervalle - ultra proche de 2

# Courbes
plt.plot(x1, h(x1), 'b-', linewidth=2, label=r'$h(x) = \ln(x^2 - 3x + 2)$')
plt.plot(x2, h(x2), 'b-', linewidth=2)

# Asymptotes verticales
plt.axvline(x=1, color='red', linestyle='--', linewidth=1.5, label='Asymptote $x = 1$')
plt.axvline(x=2, color='red', linestyle='--', linewidth=1.5, label='Asymptote $x = 2$')
...
plt.ylabel('h(x)', fontsize=14, labelpad=15, rotation=0)
...
# Sauvegarde
plt.savefig('exercice_logarithme_etude_question3.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_logarithme_etude_question3.png' créé avec succès!")