...
# Ligne horizontale y = √3/2
y_target = np.sqrt(3)/2
plt.axhline(y=y_target, color='green', linestyle='--', linewidth=1.5, label=r'$y = \frac{\sqrt{3}}{2}$')

# Solutions : x = π/3 et x = 2π/3
x1 = np.pi/3
x2 = 2*np.pi/3

# Marquer les points de solution
plt.plot(x1, y_target, 'ro', markersize=8, markeredgecolor='darkred', markeredgewidth=2, label=r'Solutions : $x = \frac{\pi}{3}, \frac{2\pi}{3}$')
plt.plot(x2, y_target, 'ro', markersize=8, markeredgecolor='darkred', markeredgewidth=2)

# Zone solution : [π/3, 2π/3]
x_solution = np.linspace(x1, x2, 100)
y_solution = np.sin(x_solution)
plt.fill_between(x_solution, y_solution, y_target, alpha=0.3, color='green', label=r'Solution : $\sin(x) \geq \frac{\sqrt{3}}{2}$')
...
# Sauvegarder
plt.savefig('inequations_optimisation_question1.png', dpi=300, bbox_inches='tight')
print("Graphique 'inequations_optimisation_question1.png' créé avec succès!")
