...
# Domaine de définition (x + 1 > 0, donc x > -1)
x = np.linspace(-0.9, 20, 1500)

# Courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = e^{\ln(x+1)}$')

# Ligne y = 3
plt.axhline(y=3, color='red', linestyle='--', linewidth=2, label='$y = 3$')

# Solution
plt.plot(2, 3, 'go', markersize=8, label='Solution $x = 2$')
...
# Sauvegarde
plt.savefig('exercice_exponentielle_logarithmiques_question2.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_logarithmiques_question2.png' créé avec succès!")
