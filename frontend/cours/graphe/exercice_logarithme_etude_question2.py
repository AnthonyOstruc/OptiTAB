...
x = np.linspace(0.001, 20, 1500)

# Courbe principale
plt.plot(x, g(x), 'b-', linewidth=2, label=r'$g(x) = x \ln(x)$')

# Point minimum
x_min = 1/np.e
y_min = g(x_min)
plt.plot(x_min, y_min, 'ro', markersize=8, label=f'Minimum en $x = \\frac{{1}}{{e}}$ : $g(\\frac{{1}}{{e}}) = -\\frac{{1}}{{e}}$')
...
plt.ylabel('g(x)', fontsize=14, labelpad=15, rotation=0)
...
# Sauvegarde
plt.savefig('exercice_logarithme_etude_question2.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_logarithme_etude_question2.png' créé avec succès!")
