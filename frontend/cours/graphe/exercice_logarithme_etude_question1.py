...
# Domaine de définition (x > 1/2)
# Echantillonnage très dense au voisinage de l'asymptote x = 1/2
near_asymptote = 0.5 + np.geomspace(1e-6, 1.5, 900)  # de 0.500001 à 2.0 environ
far_segment = np.linspace(2.0, 20.0, 600)
x = np.concatenate([near_asymptote, far_segment])

# Courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = \ln(2x - 1)$')

# Asymptote verticale
plt.axvline(x=0.5, color='red', linestyle='--', linewidth=2, label='Asymptote verticale $x = \\frac{1}{2}$')
...
# Sauvegarde
plt.savefig('exercice_logarithme_etude_question1.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_logarithme_etude_question1.png' créé avec succès!")
