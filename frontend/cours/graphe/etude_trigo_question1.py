...
plt.plot(x, f(x), 'b-', linewidth=1.5, label=r'$f(x) = \sin(x) + \cos(x)$')

# Points d'extremums
max_x1 = np.pi/4
max_x2 = 2*np.pi
min_x = 5*np.pi/4
plt.plot(max_x1, f(max_x1), 'go', markersize=6, markeredgecolor='darkgreen', markeredgewidth=1, label='Max en $\\frac{\\pi}{4}, 2\\pi$')
plt.plot(max_x2, f(max_x2), 'go', markersize=6, markeredgecolor='darkgreen', markeredgewidth=1)
plt.plot(min_x, f(min_x), 'ro', markersize=6, markeredgecolor='darkred', markeredgewidth=1, label='Min en $\\frac{5\\pi}{4}$')
...
# Sauvegarder
plt.savefig('etude_trigo_question1.png', dpi=300, bbox_inches='tight')
print("Graphique 'etude_trigo_question1.png' créé avec succès!")
