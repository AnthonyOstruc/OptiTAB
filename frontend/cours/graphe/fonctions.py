...
# Intervalles
x1 = np.linspace(-60, -3.1, 1000)
x2 = np.linspace(-2.9, 70, 1000)

# Courbes
plt.plot(x1, f(x1), 'r-', linewidth=2, label=r'$f(x) = \frac{2x + 1}{x + 3}$')
plt.plot(x2, f(x2), 'r-', linewidth=2)

# Asymptotes
plt.axvline(x=-3, color='black', linestyle='--', linewidth=2, alpha=0.7, label='Asymptote verticale x = -3')
plt.axhline(y=2, color='black', linestyle='--', linewidth=2, alpha=0.7, label='Asymptote horizontale y = 2')
...
for x in xticks_major:
    ax.plot([x, x], [-0.3, 0.3], color="black", linewidth=1.5)  # un peu plus longues que petites
    ax.text(x, -1.0, str(x), ha='center', va='top', fontsize=12)

# Axe Y grandes graduations (raccourcies)
for y in yticks_major:
    if y < 19:  # évite chevauchement avec la flèche
        ax.plot([-0.2, 0.2], [y, y], color="black", linewidth=1.5)  # <- réduit la longueur
