...
# Courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = e^{x^2 - 4x + 3}$')
...
# Solutions
x1, x2 = 1, 3
plt.plot(x1, 1, 'go', markersize=8, label=f'Solution $x = 1$')
plt.plot(x2, 1, 'go', markersize=8, label=f'Solution $x = 3$')

# Zone où f(x) >= 1
x_left = x[x <= 1]
x_right = x[x >= 3]
ax.fill_between(x_left, f(x_left), 1, where=(f(x_left) >= 1), 
                color='green', alpha=0.2, label=r'$f(x) \geq 1$')
ax.fill_between(x_right, f(x_right), 1, where=(f(x_right) >= 1), 
...
# Sauvegarde
plt.savefig('exercice_exponentielle_avancees_question5.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_avancees_question5.png' créé avec succès!")
