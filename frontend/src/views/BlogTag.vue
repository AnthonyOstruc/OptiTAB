  ...
  <MainLayout>
    <main class="blog-page">
      ...
      <nav class="breadcrumb" aria-label="Fil d'Ariane">
        ...
        <span class="breadcrumb__sep">/</span>
        <span class="breadcrumb__current">Tag : {{ tagName }}</span>
      </nav>

      <header class="blog-header">
        <h1 class="blog-title">#{{ tagName }}</h1>
      </header>
...
import { getBlogPosts, getBlogTagDetail } from '@/api/blog'
...
function applySeo(tag) {
  const name = tag?.nom || tagName.value
  const slug = route.params.slug
  const metaDesc = tag?.meta_description || `Articles avec le tag ${name} sur le blog OptiTAB.`
  const robots = tag?.meta_robots === 'noindex'
    ? 'noindex,follow'
...
async function fetchPosts() {
  ...
  loading.value = true
  try {
    const { data: tag } = await getBlogTagDetail(slug)
    tagMeta.value = tag
    tagName.value = tag?.nom || slug
    applySeo(tag)

    const { data } = await getBlogPosts({ tag: slug, page: currentPage.value, page_size: pageSize })
...
.blog-card__category { background: #eef2ff; color: #3730a3; padding: 2px 10px; border-radius: 999px; font-weight: 700; font-size: 11px; }
