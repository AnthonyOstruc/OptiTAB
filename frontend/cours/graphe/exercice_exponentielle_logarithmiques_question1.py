...
# Fonction
def f(x):
    return np.log(np.exp(2*x) - 1)

# Domaine de définition (e^(2x) - 1 > 0, donc x > 0)
x = np.linspace(0.1, 20, 1500)

# Courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = \ln(e^{2x} - 1)$')

# Ligne y = 2
plt.axhline(y=2, color='red', linestyle='--', linewidth=2, label='$y = 2$')

# Solution
x_sol = 0.5 * np.log(np.exp(2) + 1)
plt.plot(x_sol, 2, 'go', markersize=8, label=f'Solution $x = \\frac{{1}}{{2}}\\ln(e^2 + 1)$')
...
# Sauvegarde
plt.savefig('exercice_exponentielle_logarithmiques_question1.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_logarithmiques_question1.png' créé avec succès!")
