...
# Domaine de définition (x^2 - 1 > 0 et x + 1 > 0, donc x > 1)
x = np.linspace(1.1, 20, 1500)

# Courbes
plt.plot(x, f1(x), 'b-', linewidth=2, label=r'$f_1(x) = \ln(x^2 - 1)$')
plt.plot(x, f2(x), 'r-', linewidth=2, label=r'$f_2(x) = \ln(x + 1)$')

# Solution
plt.plot(2, np.log(3), 'go', markersize=8, label='Solution $x = 2$')
...
# Sauvegarde
plt.savefig('exercice_exponentielle_logarithmiques_question3.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_logarithmiques_question3.png' créé avec succès!")
