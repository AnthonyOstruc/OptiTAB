...
x = np.linspace(0.00000000001, 15, 20000)

# Tracer la courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = \ln(x) - x + 2$')
...
x_max = 1
y_max = f(x_max)
plt.plot(x_max, y_max, 'ro', markersize=8, label=f'Maximum (1, {y_max:.0f})')

# Points d'annulation (approximatifs)
x1 = 0.16  # Première solution
x2 = 3.15  # Deuxième solution
plt.plot(x1, 0, 'go', markersize=8, label=f'Zéro 1 ({x1:.2f}, 0)')
plt.plot(x2, 0, 'go', markersize=8, label=f'Zéro 2 ({x2:.2f}, 0)')
...
# Sauvegarder
plt.savefig('exercice_logarithme_synthese2_question2.png', dpi=300, bbox_inches='tight')

# Afficher le graphique
plt.show()

print("Graphique 'exercice_logarithme_synthese2_question2.png' créé avec succès!")
