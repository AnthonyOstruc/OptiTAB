...
# Fonction
def f(x):
    return np.exp(2*x) - 5*np.exp(x) + 6

# Domaine de définition
x = np.linspace(-20, 20, 1500)

# Courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = e^{2x} - 5e^x + 6$')

# Axe des zéros
plt.axhline(y=0, color='black', linestyle='-', linewidth=1, alpha=0.7, label='Axe des zéros $y = 0$')

# Solutions
x1, x2 = np.log(2), np.log(3)
plt.plot(x1, 0, 'ro', markersize=8, label=f'Solution $x = \\ln(2)$')
plt.plot(x2, 0, 'ro', markersize=8, label=f'Solution $x = \\ln(3)$')
...
x_zoom = np.linspace(0, 1.5, 200)
ax_zoom.plot(x_zoom, f(x_zoom), 'b-', linewidth=2, label=r'$f(x)$')
ax_zoom.axhline(y=0, color='black', linestyle='-', linewidth=1, alpha=0.7)
ax_zoom.plot(np.log(2), 0, 'ro', markersize=6, label=r'$x = \ln(2)$')
ax_zoom.plot(np.log(3), 0, 'ro', markersize=6, label=r'$x = \ln(3)$')

# Configuration du zoom
ax_zoom.set_xlim(0, 1.5)
...
# Sauvegarde
plt.savefig('exercice_exponentielle_avancees_question1.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_avancees_question1.png' créé avec succès!")
