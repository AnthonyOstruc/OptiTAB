...
# Courbes
plt.plot(x, f1(x), 'b-', linewidth=2, label=r'$y = \cos x$')
plt.plot(x, f2(x), 'r-', linewidth=2, label=r'$y = x$')

# Points clés sur [0, π/2]
plt.plot(0, np.cos(0), 'go', markersize=6)
ax.annotate('$f(0) = 1 > 0$', (0, np.cos(0)), xytext=(-7, 3), fontsize=10,
            arrowprops=dict(arrowstyle='->', color='green', lw=1))

plt.plot(np.pi/2, np.cos(np.pi/2), 'go', markersize=6)
ax.annotate('$f(\\pi/2) < 0$', (np.pi/2, np.cos(np.pi/2)), xytext=(4, -3), fontsize=10,
            arrowprops=dict(arrowstyle='->', color='green', lw=1))
...
# Sauvegarde
plt.savefig('/Users/anthonytabet/Desktop/OptiTABV2/OptiTAB/frontend/cours/graphe/tvi_question4.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'tvi_question4.png' créé avec succès!")
