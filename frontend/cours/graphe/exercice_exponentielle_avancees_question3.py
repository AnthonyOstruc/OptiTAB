...
# Courbe principale
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = e^{2x} - 3e^x + 2$')

# Solutions
x1, x2 = 0, np.log(2)
plt.plot(x1, 0, 'ro', markersize=8, label=f'Solution $x = 0$')
plt.plot(x2, 0, 'ro', markersize=8, label=f'Solution $x = \\ln(2)$')

# Zone où f(x) <= 0
x_zone = x[(x >= 0) & (x <= np.log(2))]
ax.fill_between(x_zone, f(x_zone), 0, where=(f(x_zone) <= 0), 
                color='green', alpha=0.2, label=r'$f(x) \leq 0$')
...
x_zoom = np.linspace(-0.5, 1, 200)
ax_zoom.plot(x_zoom, f(x_zoom), 'b-', linewidth=2, label=r'$f(x)$')
ax_zoom.plot(0, 0, 'ro', markersize=6, label=r'$x = 0$')
ax_zoom.plot(np.log(2), 0, 'ro', markersize=6, label=r'$x = \ln(2)$')

# Zone où f(x) <= 0 dans le zoom
x_zone_zoom = x_zoom[(x_zoom >= 0) & (x_zoom <= np.log(2))]
ax_zoom.fill_between(x_zone_zoom, f(x_zone_zoom), 0, where=(f(x_zone_zoom) <= 0), 
                     color='green', alpha=0.2)

# Configuration du zoom
ax_zoom.set_xlim(-0.5, 1)
...
# Sauvegarde
plt.savefig('exercice_exponentielle_avancees_question3.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_avancees_question3.png' créé avec succès!")
