...
# Courbes
plt.plot(x1, f(x1), 'b-', linewidth=2, label=r'$f(x) = \frac{x^2 - 1}{x - 2}$')
plt.plot(x2, f(x2), 'b-', linewidth=2)

# Asymptote verticale x = 2
plt.axvline(x=2, color='red', linestyle='--', linewidth=1.5, alpha=0.7, label='Asymptote verticale $x = 2$')

# Asymptote oblique y = x + 2
x_asymptote = np.linspace(-20, 20, 1000)
plt.plot(x_asymptote, asymptote_oblique(x_asymptote), 'green', linestyle='--', linewidth=1.5, alpha=0.7, label='Asymptote oblique $y = x + 2$')
...
# Sauvegarde
plt.savefig('/Users/anthonytabet/Desktop/OptiTABV2/OptiTAB/frontend/cours/graphe/asymptotes_question1.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'asymptotes_question1.png' créé avec succès!")
