...
# Courbes
plt.plot(x, f1(x), 'b-', linewidth=2, label=r'$y = e^{x-1}$')
plt.plot(x, f2(x), 'r-', linewidth=2, label=r'$y = e^{2x+3}$')

# Zone où f1(x) < f2(x) (solution de l'inéquation)
x_zone = np.linspace(-4, 20, 1000)
plt.fill_between(x_zone, f1(x_zone), f2(x_zone), alpha=0.3, color='green', 
                 where=(f1(x_zone) < f2(x_zone)), label='$e^{x-1} < e^{2x+3}$')

# Point d'intersection (limite de la zone)
plt.plot(-4, np.exp(-5), 'go', markersize=6, label='Limite $x = -4$')
...
# Sauvegarde
plt.savefig('exercice_exponentielle_equations_question4.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_equations_question4.png' créé avec succès!")
