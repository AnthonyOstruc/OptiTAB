...
# Fonction m(x) définie par morceaux
x1 = np.linspace(-20, 1, 1500)
x2 = np.linspace(1, 20, 1500)

# Première partie: x² pour x < 1
y1 = x1**2
plt.plot(x1[:-1], y1[:-1], 'b-', linewidth=2, label=r'$m(x)$ (fonction par morceaux)')

# Deuxième partie: x + 1 pour x ≥ 1
y2 = x2 + 1
plt.plot(x2, y2, 'b-', linewidth=2)

# Limite à gauche en x = 1: y = 1
plt.plot(1, 1, 'ro', markersize=8, markerfacecolor='white', markeredgewidth=2, label='Limite à gauche = 1')

# Valeur de la fonction et limite à droite en x = 1: y = 2
plt.plot(1, 2, 'go', markersize=8, label='$m(1) = 2$')

# Annotation du saut
ax.annotate('Saut d\'amplitude 1', (1, 1.5), xytext=(4, 4), fontsize=10,
...
# Sauvegarde
plt.savefig('/Users/anthonytabet/Desktop/OptiTABV2/OptiTAB/frontend/cours/graphe/discontinuites_question5.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'discontinuites_question5.png' créé avec succès!")
