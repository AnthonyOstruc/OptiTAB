...
plt.plot(x, f2(x), 'r-', linewidth=2, label=r'$y = 2e^{x-1}$')

# Zone où f1(x) > f2(x) (toujours vraie)
ax.fill_between(x, f1(x), f2(x), where=(f1(x) > f2(x)), 
                color='green', alpha=0.2, label=r'$e^{x+1} > 2e^{x-1}$ (toujours)')
...
# Sauvegarde
plt.savefig('exercice_exponentielle_avancees_question4.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'exercice_exponentielle_avancees_question4.png' créé avec succès!")
