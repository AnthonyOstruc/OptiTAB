...
# Courbe
plt.plot(x, f(x), 'b-', linewidth=2, label=r'$f(x) = x^4 - 2x^2 + 1 = (x^2 - 1)^2$')

# Axe y = 0
plt.axhline(y=0, color='green', linestyle='--', linewidth=1.5, alpha=0.7, label='$y = 0$')

# Points solutions
plt.plot(-1, 0, 'ro', markersize=8, label='Solutions en $x = \\pm 1$')
plt.plot(1, 0, 'ro', markersize=8)

ax.annotate('$f(-1) = 0$', (-1, 0), xytext=(-6, 3), fontsize=10,
            arrowprops=dict(arrowstyle='->', color='red', lw=1))
ax.annotate('$f(1) = 0$', (1, 0), xytext=(4, 3), fontsize=10,
            arrowprops=dict(arrowstyle='->', color='red', lw=1))

# Points aux bornes
plt.plot(-2, f(-2), 'go', markersize=6)
ax.annotate('$f(-2) = 9$', (-2, f(-2)), xytext=(-8, 12), fontsize=10,
            arrowprops=dict(arrowstyle='->', color='green', lw=1))

plt.plot(2, f(2), 'go', markersize=6)
ax.annotate('$f(2) = 9$', (2, f(2)), xytext=(5, 12), fontsize=10,
            arrowprops=dict(arrowstyle='->', color='green', lw=1))
...
# Sauvegarde
plt.savefig('/Users/anthonytabet/Desktop/OptiTABV2/OptiTAB/frontend/cours/graphe/tvi_question5.png', 
            dpi=300, bbox_inches='tight')
plt.show()

print("Graphique 'tvi_question5.png' créé avec succès!")
